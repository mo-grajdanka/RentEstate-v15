...
function fillPreview(site) {
  ...
  // если это демонстрация для "Площадка B"
  if (site.name === "Площадка B") {
    cardEl.removeAttribute("href"); // уберём кликабельность всей карточки
    cardEl.innerHTML = `
  ...
  // стандартный вид для всех остальных
  const img0 = site.images?.[0] || "";
  const area = Number.isFinite(site.area) ? site.area.toLocaleString("ru-RU") : (site.area ?? "—");
  ...
  cardEl.innerHTML = `
    <img src="${img0}" alt="${site.name || ""}"
         ...
         onerror="this.src='https://placehold.co/900x256?text=No+Image'">
    <div class="p-6 space-y-3">
      <h3 class="text-2xl font-semibold">${site.name || ""}</h3>
      ${site.place ? `<p class="text-gray-600">${site.place}</p>` : ""}
      <p class="text-gray-800">Площадь: ${Number(site.area).toLocaleString("ru-RU")} м²</p>
      ${site.hoverText ? `<p class="text-gray-700 leading-relaxed">${site.hoverText}</p>` : ""}
...
function hidePreview() {
 ...
 // Если кликнули по ссылке «Подробнее», просто переходим по ссылке.
